(function() {
  var assert, dynamics, expect, expectEqualMatrix3d, jsdom, mocha;

  mocha = require('mocha');

  jsdom = require('mocha-jsdom');

  expect = require('chai').expect;

  assert = require('chai').assert;

  dynamics = require('../src/dynamics');

  jsdom();

  dynamics.tests = {
    matrixForTransform: function(transform) {
      if (transform === "translateX(50px) rotateZ(45deg)") {
        return "matrix3d(0.7071067811865476, 0.7071067811865475, 0, 0, -0.7071067811865475, 0.7071067811865476, 0, 0, 0, 0, 1, 0, 50, 0, 0, 1)";
      }
    }
  };

  expectEqualMatrix3d = function(a, b) {
    var argsA, argsB, i, r, _i, _ref, _ref1, _ref2, _results;
    r = /matrix3?d?\(([-0-9, \.]*)\)/;
    argsA = (_ref = a.match(r)) != null ? _ref[1].split(',') : void 0;
    argsB = (_ref1 = b.match(r)) != null ? _ref1[1].split(',') : void 0;
    _results = [];
    for (i = _i = 0, _ref2 = argsA.length; 0 <= _ref2 ? _i < _ref2 : _i > _ref2; i = 0 <= _ref2 ? ++_i : --_i) {
      _results.push(expect(Math.abs(parseFloat(argsA[i]) - parseFloat(argsB[i]))).to.be.below(0.00001));
    }
    return _results;
  };

  describe('dynamics.css', function() {
    it('apply css to a DOM element', function() {
      var el;
      el = document.createElement('div');
      dynamics.css(el, {
        left: 0,
        top: "5px",
        backgroundColor: "#FF0000"
      });
      expect(el.style.left).eql('0px');
      expect(el.style.top).eql('5px');
      return expect(el.style.backgroundColor).eql('rgb(255, 0, 0)');
    });
    it('apply transform to a DOM element', function() {
      var el;
      el = document.createElement('div');
      dynamics.css(el, {
        translateX: 10,
        translateY: "0px",
        translateZ: "25%",
        rotateZ: "90deg",
        rotateX: 45,
        skewX: 10,
        scale: 2
      });
      return expect(el.style.transform).eql("translateX(10px) translateY(0px) translateZ(25%) rotateZ(90deg) rotateX(45deg) skewX(10deg) scaleX(2) scaleY(2) scaleZ(2)");
    });
    return it('works with an array of DOM element', function() {
      var els;
      els = [document.createElement('div'), document.createElement('div')];
      dynamics.css(els, {
        left: "10px"
      });
      expect(els[0].style.left).eql('10px');
      return expect(els[1].style.left).eql('10px');
    });
  });

  describe('dynamics.animate', function() {
    it('animate position properties of a DOM element', function(done) {
      var el;
      el = document.createElement('div');
      dynamics.animate(el, {
        left: 100,
        top: "50px",
        translateX: 50,
        rotateZ: 45
      }, {
        duration: 25,
        type: dynamics.easeInOut
      });
      return setTimeout(function() {
        expect(el.style.left).eql("100px");
        expect(el.style.top).eql("50px");
        expectEqualMatrix3d(el.style.transform, dynamics.tests.matrixForTransform("translateX(50px) rotateZ(45deg)"));
        return done();
      }, 50);
    });
    it('animate with a delay', function(done) {
      var el;
      el = document.createElement('div');
      el.style.left = 0;
      dynamics.animate(el, {
        left: 100
      }, {
        duration: 25,
        delay: 100,
        type: dynamics.easeInOut
      });
      setTimeout(function() {
        return expect(el.style.left).eql("0px");
      }, 50);
      setTimeout(function() {
        return expect(el.style.left).not.eql("100px");
      }, 110);
      return setTimeout(function() {
        expect(el.style.left).eql("100px");
        return done();
      }, 150);
    });
    it('works with an array of elements', function(done) {
      var el0asserted, el1asserted, els;
      els = [document.createElement('div'), document.createElement('div')];
      el0asserted = false;
      el1asserted = false;
      dynamics.animate(els, {
        left: 100
      }, {
        duration: 25,
        complete: function(el) {
          if (el === els[0]) {
            el0asserted = true;
          }
          if (el === els[1]) {
            return el1asserted = true;
          }
        }
      });
      return setTimeout(function() {
        expect(els[0].style.left).eql("100px");
        expect(els[1].style.left).eql("100px");
        assert(el0asserted, "complete wasn't called with the right element");
        assert(el1asserted, "complete wasn't called with the right element");
        return done();
      }, 50);
    });
    it('calls change while the animation is running', function(done) {
      var changeCalls, el;
      el = document.createElement('div');
      changeCalls = 0;
      dynamics.animate(el, {
        left: 100,
        top: "50px"
      }, {
        duration: 100,
        type: dynamics.easeInOut,
        change: function() {
          return changeCalls += 1;
        }
      });
      return setTimeout(function() {
        expect(changeCalls).to.be.above(1);
        return done();
      }, 150);
    });
    it('actually animates properties while the animation is running', function(done) {
      var el, previous;
      el = document.createElement('div');
      previous = {
        left: 0,
        top: 0,
        translateX: 0,
        rotateZ: 0,
        transform: 'none'
      };
      dynamics.animate(el, {
        left: 100,
        top: "50px",
        translateX: 50,
        rotateZ: 45
      }, {
        duration: 100,
        type: dynamics.easeInOut
      });
      setInterval(function() {
        var current;
        current = {
          left: parseFloat(el.style.left),
          top: parseFloat(el.style.top),
          transform: el.style.transform
        };
        assert(current.left >= previous.left, "Left should increment");
        assert(current.top >= previous.top, "Top should increment");
        assert(current.transform !== previous.transform || (current.transform === previous.transform && current.transform !== "none"), "Transform should change");
        return previous = current;
      }, 20);
      return setTimeout(function() {
        return done();
      }, 150);
    });
    it('calls complete when the animation is over', function(done) {
      var el;
      el = document.createElement('div');
      return dynamics.animate(el, {
        left: 100,
        top: "50px"
      }, {
        duration: 25,
        complete: function() {
          return done();
        }
      });
    });
    it('comes back to the original value with dynamics.bounce', function(done) {
      var el;
      el = document.createElement('div');
      return dynamics.animate(el, {
        left: 100
      }, {
        duration: 25,
        type: dynamics.bounce,
        complete: function() {
          expect(el.style.left).eql("0px");
          return done();
        }
      });
    });
    it('animates the points of a svg element correctly', function(done) {
      var el, previous, regex;
      regex = /M([.\d]*),([.\d]*) C([.\d]*),([.\d]*)/;
      dynamics.tests.isSVG = function(el) {
        return true;
      };
      el = document.createElement('polygon');
      el.setAttribute("points", "M101.88,22 C101.88,18.25");
      previous = el.getAttribute("points").match(regex);
      dynamics.animate(el, {
        points: "M50,10 C88.11,20.45"
      }, {
        duration: 100
      });
      setInterval(function() {
        var current;
        current = el.getAttribute("points").match(regex);
        assert(current != null);
        expect(parseFloat(current[1])).to.at.most(parseFloat(previous[1]));
        expect(parseFloat(current[2])).to.at.most(parseFloat(previous[2]));
        expect(parseFloat(current[3])).to.at.most(parseFloat(previous[3]));
        expect(parseFloat(current[4])).to.at.least(parseFloat(previous[4]));
        return previous = current;
      }, 20);
      return setTimeout(function() {
        expect(el.getAttribute("points")).to.be.equal("M50,10 C88.11,20.45");
        return done();
      }, 150);
    });
    it('animates properties of a object correctly', function(done) {
      var assertFormats, assertTypes, object, previous;
      assertTypes = function(object) {
        expect(typeof object.number).to.be.equal('number', 'object.number has the wrong type');
        expect(typeof object.negative).to.be.equal('number', 'object.negative has the wrong type');
        expect(typeof object.string).to.be.equal('string', 'object.string has the wrong type');
        expect(typeof object.stringArray).to.be.equal('string', 'object.stringArray has the wrong type');
        assert(object.array instanceof Array, 'object.array has the wrong type');
        expect(typeof object.hexColor).to.be.equal('string', 'object.hexColor has the wrong type');
        expect(typeof object.rgbColor).to.be.equal('string', 'object.rgbColor has the wrong type');
        return expect(typeof object.rgbaColor).to.be.equal('string', 'object.rgbaColor has the wrong type');
      };
      assertFormats = function(object) {
        assert(object.stringArray.match(/^([.\d]*) ([.\d]*), d([.\d]*):([.\d]*)$/) != null, 'object.stringArray has the wrong format');
        assert(object.array[0].match(/^([.\d]*)deg$/) != null, 'object.array[0] has the wrong format');
        assert(object.array[2].match(/^([.\d]*)$/) != null, 'object.array[2] has the wrong format');
        assert(object.array[4].match(/^#([a-zA-Z\d]{6})$/) != null, 'object.array[4] has the wrong format');
        assert(object.hexColor.match(/^#([a-zA-Z\d]{6})$/) != null, 'object.hexColor has the wrong format');
        assert(object.rgbColor.match(/^rgb\(([.\d]*), ([.\d]*), ([.\d]*)\)$/) != null, 'object.rgbColor has the wrong format');
        return assert(object.rgbaColor.match(/^rgba\(([.\d]*), ([.\d]*), ([.\d]*), ([.\d]*)\)$/) != null, 'object.rgbaColor has the wrong format');
      };
      object = {
        number: 0,
        negative: -10,
        string: "10",
        stringArray: "10 50, d10:50",
        array: ["0deg", 0, "1.10", 10, "#FFFFFF"],
        hexColor: "#FFFFFF",
        rgbColor: "rgb(255, 255, 255)",
        rgbaColor: "rgba(255, 255, 255, 0)"
      };
      previous = JSON.parse(JSON.stringify(object));
      dynamics.animate(object, {
        number: 10,
        negative: 50,
        string: "50",
        stringArray: "100 1, d0:100",
        array: ["100deg", 40, "2.20", 20, "#123456"],
        hexColor: "#123456",
        rgbColor: "rgb(18, 52, 86)",
        rgbaColor: "rgba(18, 52, 86, 1)"
      }, {
        duration: 100
      });
      setInterval(function() {
        var current, previousStringArrayArgs, stringArrayArgs;
        current = JSON.parse(JSON.stringify(object));
        assertTypes(current);
        assertFormats(current);
        expect(current.number).to.at.least(previous.number);
        expect(current.negative).to.at.least(previous.negative);
        expect(parseFloat(current.string)).to.at.least(parseFloat(previous.string));
        stringArrayArgs = current.stringArray.match(/^([.\d]*) ([.\d]*), d([.\d]*):([.\d]*)$/);
        previousStringArrayArgs = previous.stringArray.match(/^([.\d]*) ([.\d]*), d([.\d]*):([.\d]*)$/);
        expect(parseFloat(stringArrayArgs[1])).to.at.least(parseFloat(previousStringArrayArgs[1]));
        expect(parseFloat(stringArrayArgs[2])).to.at.most(parseFloat(previousStringArrayArgs[2]));
        expect(parseFloat(stringArrayArgs[3])).to.at.most(parseFloat(previousStringArrayArgs[3]));
        expect(parseFloat(stringArrayArgs[4])).to.at.least(parseFloat(previousStringArrayArgs[4]));
        return previous = current;
      }, 20);
      return setTimeout(function() {
        assertTypes(object);
        assertFormats(object);
        return done();
      }, 150);
    });
    return it('animates actual properties of an object correctly', function(done) {
      var object, previousProp;
      object = {};
      Object.defineProperty(object, "prop", {
        set: function(v) {
          return this._prop = v;
        },
        get: function() {
          return this._prop;
        }
      });
      object.prop = 1;
      dynamics.animate(object, {
        prop: 0
      }, {
        duration: 100
      });
      previousProp = object.prop;
      setInterval(function() {
        assert(object.prop >= 0 && object.prop <= 1, "prop is between 0 and 1");
        assert(object.prop < previousProp || object.prop === 0, "prop should be decreasing or equal 0");
        return previousProp = object.prop;
      }, 20);
      return setTimeout(function() {
        expect(object.prop).to.be.equal(0, 'object.prop has the wrong end value');
        return done();
      }, 150);
    });
  });

  describe('dynamics.stop', function() {
    it('actually stops current animation', function(done) {
      var changeCanBeCalled, el;
      el = document.createElement('div');
      changeCanBeCalled = true;
      dynamics.animate(el, {
        left: 100
      }, {
        duration: 100,
        type: dynamics.easeInOut,
        change: function() {
          return assert(changeCanBeCalled, "change shouldn't be called anymore");
        },
        complete: function() {
          return assert(false, "complete shouldn't be called");
        }
      });
      setTimeout(function() {
        dynamics.stop(el);
        return changeCanBeCalled = false;
      }, 50);
      return setTimeout(function() {
        return done();
      }, 150);
    });
    return it('also works with a delayed animation', function(done) {
      var el;
      el = document.createElement('div');
      dynamics.animate(el, {
        left: 100
      }, {
        duration: 100,
        delay: 100,
        change: function() {
          return assert(false, "change shouldn't be called");
        },
        complete: function() {
          return assert(false, "complete shouldn't be called");
        }
      });
      setTimeout(function() {
        return dynamics.stop(el);
      }, 50);
      return setTimeout(function() {
        return done();
      }, 150);
    });
  });

  describe('curves', function() {
    describe('dynamics.linear', function() {
      return it('works', function() {
        var curve;
        curve = dynamics.linear();
        expect(curve(0)).eql(0);
        expect(curve(0.1)).eql(0.1);
        expect(curve(5)).eql(5);
        expect(curve(5.6)).eql(5.6);
        expect(curve(7.8)).eql(7.8);
        return expect(curve(1)).eql(1);
      });
    });
    describe('dynamics.easeInOut', function() {
      return it('works', function() {
        var curve;
        curve = dynamics.easeInOut();
        expect(curve(0)).eql(0);
        assert(curve(0.25) > 0 && curve(0.25) < 0.5);
        expect(curve(0.5)).eql(0.5);
        assert(curve(0.75) > 0.5 && curve(0.75) < 1);
        return expect(curve(1)).eql(1);
      });
    });
    describe('dynamics.easeIn', function() {
      return it('increases exponentially', function() {
        var curve, diff, i, inter, newDiff, t1, t2, _i, _results;
        curve = dynamics.easeIn();
        inter = 0.1;
        diff = 0;
        _results = [];
        for (i = _i = 1; _i <= 10; i = ++_i) {
          t1 = inter * (i - 1);
          t2 = inter * i;
          newDiff = curve(t2) - curve(t1);
          assert(newDiff > diff, "easeIn should be exponential");
          _results.push(diff = newDiff);
        }
        return _results;
      });
    });
    describe('dynamics.easeOut', function() {
      return it('increases 1/exponentially', function() {
        var curve, diff, i, inter, newDiff, t1, t2, _i, _results;
        curve = dynamics.easeOut();
        inter = 0.1;
        diff = 1;
        _results = [];
        for (i = _i = 1; _i <= 10; i = ++_i) {
          t1 = inter * (i - 1);
          t2 = inter * i;
          newDiff = curve(t2) - curve(t1);
          assert(newDiff < diff, "easeOut should be 1/exponential");
          _results.push(diff = newDiff);
        }
        return _results;
      });
    });
    describe('dynamics.bounce', function() {
      return it('starts and returns to initial state', function() {
        var curve;
        curve = dynamics.bounce();
        assert(curve(0) < 0.001 && curve(0) >= 0);
        return assert(curve(1) < 0.001 && curve(1) >= 0);
      });
    });
    return describe('dynamics.forceWithGravity', function() {
      return it('starts and returns to initial state', function() {
        var curve;
        curve = dynamics.forceWithGravity();
        assert(curve(0) < 0.001 && curve(0) >= 0);
        return assert(curve(1) < 0.001 && curve(1) >= 0);
      });
    });
  });

  describe('dynamics.setTimeout', function() {
    return it('works', function(done) {
      var t;
      t = Date.now();
      return dynamics.setTimeout(function() {
        assert(Math.abs(Date.now() - t - 100) < 30);
        return done();
      }, 100);
    });
  });

  describe('dynamics.clearTimeout', function() {
    return it('works', function(done) {
      var i;
      i = dynamics.setTimeout(function() {
        return assert(false);
      }, 100);
      dynamics.clearTimeout(i);
      return setTimeout(function() {
        return done();
      }, 200);
    });
  });

}).call(this);
